...
final class GalleryUITests: XCTestCase {
  ...
  func testLastCompanionRemainsSelectedAfterViewer() {
    ...
    // iOS 26 adds vertical content insets to the chip selector, so only the
    // horizontal placement inside the selector is asserted.
    let selector = app.scrollViews["gallery.companions"]
    XCTAssertGreaterThanOrEqual(selected.frame.minX, selector.frame.minX - 1)
    XCTAssertLessThanOrEqual(selected.frame.maxX, selector.frame.maxX + 1)
  }

  func testGalleryLargeTextAndReducedMotion() {
    launchGallery(largeText: true)
    // Chats accumulate across suite runs and permanently unlock moments,
    // so any fixed companion can be fully unlocked on a reused simulator.
    // Pick the first built-in companion that still has a locked moment.
    // Locked cards keep their "Locked. 5 chat exchanges…" value under the
    // All filter, so detection needs no filter round trip; scrolling the
    // filter chips back from Locked to All is unreliable at accessibility
    // text sizes.
    var character = 0
    for index in 1...8 {
      if index > 1 { chooseCompanion(index) }
      let second = moment(index, 2)
      if second.waitForExistence(timeout: 3),
        let value = second.value as? String, value.hasPrefix("Locked") {
        character = index
        break
      }
    }
    XCTAssertGreaterThan(character, 0)
    reveal(moment(character))
    capture("gallery-large-text-grid")
    moment(character).tap()
    XCTAssertTrue(app.buttons["gallery.viewer.close"].waitForExistence(timeout: 5))
    ...
    app.buttons["gallery.viewer.close"].tap()
    chooseFilter("Locked")
    reveal(moment(character, 2))
    moment(character, 2).tap()
    ...
    chooseFilter("Unlocked")
    for _ in 0..<4 where moment(character).frame.maxY > app.tabBars.firstMatch.frame.minY {
      app.swipeUp()
    }
    XCTAssertTrue(moment(character).isHittable)
    XCTAssertLessThanOrEqual(moment(character).frame.maxY, app.tabBars.firstMatch.frame.minY)
    XCTAssertGreaterThanOrEqual(
      moment(character).frame.minY, app.navigationBars["Gallery"].frame.maxY)
    capture("gallery-large-text-scrolled-card")
  }

  /// First existing row among localized label candidates; the system
  /// Settings app follows the device language (ru on this simulator).
  private func settingsText(_ settings: XCUIApplication, _ labels: [String]) -> XCUIElement {
    for label in labels {
      let row = settings.staticTexts[label].firstMatch
      if row.waitForExistence(timeout: 2) { return row }
    }
    return settings.staticTexts[labels[0]].firstMatch
  }

  /// First existing switch among localized label candidates.
  private func settingsSwitch(_ settings: XCUIApplication, _ labels: [String]) -> XCUIElement {
    for label in labels {
      let toggle = settings.switches[label].firstMatch
      if toggle.waitForExistence(timeout: 2) { return toggle }
    }
    return settings.switches[labels[0]].firstMatch
  }

  func testSystemReduceMotionGalleryFlow() {
    let settings = XCUIApplication(bundleIdentifier: "com.apple.Preferences")
    settings.launch()
    let accessibility = settingsText(settings, ["Accessibility", "Универсальный доступ"])
    for _ in 0..<6 where !accessibility.isHittable { settings.swipeUp() }
    XCTAssertTrue(accessibility.isHittable)
    accessibility.tap()
    settingsText(settings, ["Motion", "Движение"]).tap()
    let toggle = settingsSwitch(
      settings, ["Reduce Motion", "Уменьшить движение", "Уменьшение движения"])
    XCTAssertTrue(toggle.waitForExistence(timeout: 5))
    if toggle.value as? String == "0" {
      toggle.coordinate(withNormalizedOffset: CGVector(dx: 0.9, dy: 0.5)).tap()
    }
    let motionDeadline = Date().addingTimeInterval(5)
    while toggle.value as? String == "0" && Date() < motionDeadline {
