...
final class IntegrationUITests: XCTestCase {
  ...
  func testCompleteExperience() throws {
    ...
    app.launch()
    XCTAssertTrue(app.staticTexts["Meet your companion"].waitForExistence(timeout: 10))
    capture("01-onboarding")
    app.buttons["Continue"].tap()
    XCTAssertTrue(app.staticTexts["Choose the moment"].waitForExistence(timeout: 3))
    capture("cyan-onboarding-moments")
    app.buttons["Continue"].tap()
    XCTAssertTrue(app.staticTexts["Private by design"].waitForExistence(timeout: 3))
    capture("cyan-onboarding-privacy")
    app.buttons["Meet the characters"].tap()
    ...
    XCTAssertTrue(
      app.staticTexts.matching(NSPredicate(format: "label CONTAINS %@", "not configured"))
        .firstMatch.exists)
    capture("10-honest-ai-error")
    app.buttons["Retry"].tap()
    XCTAssertTrue(app.buttons["Retry"].waitForExistence(timeout: 8))
    XCTAssertEqual(
      app.staticTexts.matching(identifier: "Hello from integration verification").count, 1)
    app.navigationBars.buttons.firstMatch.tap()
    let storedConversation = button("Hello from integration verification")
  ...
  func testCompanionEditDuplicateDelete() throws {
    ...
    app.buttons["companion.confirmDelete"].tap()
    XCTAssertTrue(app.navigationBars[companionName + " Edited"].waitForExistence(timeout: 5))
    app.buttons["Delete Companion"].tap()
    XCTAssertTrue(app.buttons["companion.confirmDelete"].waitForExistence(timeout: 3))
    app.buttons["companion.confirmDelete"].tap()
  ...
  func testEditorialPortraitsAndNavigationHeadings() {
    ...
    let about = button("About Nova")
    reveal(about)
    about.tap()
    XCTAssertTrue(app.navigationBars["About"].exists)
    capture("midnight-rounded-about")
  }
  ...
  func testEditorialProfilesAndLargeTextChat() {
    for category in ["UICTContentSizeCategoryL", "UICTContentSizeCategoryAccessibilityL"] {
      ...
      XCTAssertEqual(app.staticTexts.matching(identifier: "Midnight layout verification").count, 1)
      retry.tap()
      XCTAssertTrue(retry.waitForExistence(timeout: 5))
      XCTAssertEqual(app.staticTexts.matching(identifier: "Midnight layout verification").count, 1)
      capture("midnight-chat-error-\(category)")
      app.buttons["Dismiss"].tap()
      XCTAssertFalse(app.buttons["Dismiss"].exists)
      app.scrollViews.firstMatch.swipeUp()
      reveal(retry, fullyVisible: true)
      XCTAssertEqual(app.staticTexts.matching(identifier: "Midnight layout verification").count, 1)
      capture("midnight-chat-retry-\(category)")
      app.navigationBars.buttons.firstMatch.tap()
      XCTAssertTrue(app.navigationBars["Conversations"].waitForExistence(timeout: 5))
  ...
  func testAccessibleLayout() {
    ...
    button("Continue").tap()
    button("Continue").tap()
    button("Meet the characters").tap()
    XCTAssertTrue(app.navigationBars["Discover"].waitForExistence(timeout: 5))
