...
/// TC-2: the auto language mode must always inject a language rule — pinned
/// to the script of the user's message when unambiguous, mirroring otherwise
/// — and fixed codes must still pin the reply language over any detection.
func checkLanguageInstructions() {
  guard let auto = ConversationLanguages.instruction(forCode: "") else {
    preconditionFailure("Auto language must always inject a language rule")
  }
  precondition(auto.contains("Detect the language"))
  let russian = ConversationLanguages.instruction(
    forCode: "", mirroring: "Привет! Как проходит твой день?")
  precondition(russian?.contains("Russian") == true)
  let ukrainian = ConversationLanguages.instruction(forCode: "", mirroring: "Привіт, як справи")
  precondition(ukrainian?.contains("Ukrainian") == true)
  let latin = ConversationLanguages.instruction(forCode: "", mirroring: "Hola, ¿qué tal?")
  precondition(latin?.contains("Detect the language") == true)
  guard let fixed = ConversationLanguages.instruction(forCode: "ru") else {
    preconditionFailure("A fixed language must pin the reply language")
  }
  precondition(fixed.contains("Русский"))
  let fixedOverDetection = ConversationLanguages.instruction(
    forCode: "es", mirroring: "Привет")
  precondition(fixedOverDetection?.contains("Español") == true)
  let unknown = ConversationLanguages.instruction(forCode: "unknown-code")
  precondition(unknown?.contains("Detect the language") == true)
}

@main struct Checks {
  @MainActor static func main() async throws {
    try await checkAccessIntegration()
    ...
    precondition(
      ai.requests[0].messages.last(where: { $0.role == .user })?.text == "Real harness input")
    // Auto locale ("") now injects the mirroring instruction as the trailing
    // system message instead of sending no language guidance at all.
    precondition(ai.requests[0].messages.last?.role == .system)
    precondition(
      ai.requests[0].messages.last?.text == ConversationLanguages.instruction(forCode: ""))
    ai.shouldFail = false
    ...
    // Per-turn memory extraction may add out-of-band requests; count chats.
    precondition(
    ...
    let resumed = ChatViewModel(ai: ai, localeCode: "")
    ...
    let fresh = ChatViewModel(ai: ai, localeCode: "")
    fresh.open(
  ...
  private static func checkScenarioSelection(url: URL) async throws {
    ...
    for scenario in scenarios {
      ...
      let chatRequest = ai.requests.last {
        !$0.messages[0].text.contains("list new facts about the user")
      }
      precondition(chatRequest?.scenario == scenario)
      // Request = eligible history + the new user turn + the trailing
      // auto-language system instruction that mirrors the user's language.
      precondition(chatRequest?.messages.count == messages.count + 2)
    ...
    let reopened = ChatViewModel(ai: ai, localeCode: "")
    reopened.open(
    ...
    precondition(
      ai.requests.last { !$0.messages[0].text.contains("list new facts about the user") }?
        .scenario == scenarios.last)
    ...
    let resetModel = ChatViewModel(ai: ai, localeCode: "")
    resetModel.open(
    ...
    let resetRequest = ai.requests.last {
      !$0.messages[0].text.contains("list new facts about the user")
    }
    precondition(resetRequest?.scenario == nil)
    precondition(resetRequest?.messages.count == messageIDs.count + 2)
    print(
