      ...
      ZStack {
        ...
        if let image = CharacterPortraits.image(for: character) {
          GeometryReader { geometry in
            ...
            // A wide frame (e.g. iPad) would crop a portrait to a thin band,
            // so show it whole over a blurred backdrop of itself.
            if size.width / max(size.height, 1) > imageAspect * 1.25 {
              ZStack {
                Image(uiImage: image)
                  .resizable()
                  .scaledToFill()
                  ...
                  .overlay(NovaTheme.background.opacity(0.6))
                  .clipped()
                Image(uiImage: image)
                  .resizable()
                  .scaledToFit()
