...
struct CharacterCreatorView: View {
  ...
  private func save() {
    ...
    do {
      ...
      // #Predicate cannot reference enum cases directly (it expands them into key
      // paths), so the raw value is captured in a local constant first.
      let customOriginRawValue = CharacterOrigin.custom.rawValue
      if let customCount = try? modelContext.fetchCount(
        FetchDescriptor<CharacterRecord>(
          predicate: #Predicate { $0.originRawValue == customOriginRawValue }))
      {
        _ = try? AchievementStore(context: modelContext)
          .record(kind: .customCompanions, value: customCount)
      }
