...
struct CharacterDetailView: View {
  ...
  @StateObject private var speech = RemoteVoiceSpeechService()
  ...
  var body: some View {
    ScrollView {
      VStack(spacing: NovaTheme.Spacing.sectionGap) {
        CharacterArtwork(
          character: character, height: horizontalSizeClass == .regular ? 520 : 360,
          showsName: true)
    ...
    .confirmationDialog(
      "Delete \(character.name)?", isPresented: $confirmsDelete, titleVisibility: .visible
    ) {
      Button("Delete Companion", role: .destructive, action: deleteCompanion)
        .accessibilityIdentifier("character.confirmDelete")
    } message: {
  ...
  private func deleteCompanion() {
    speech.stop()
    do {
      try SwiftDataCharacterRepository(context: modelContext).deleteCustom(id: character.id)
      dismiss()
    } catch {
  ...
  private var voiceButton: some View {
    ...
    .buttonStyle(NovaSecondaryButtonStyle())
    .controlSize(.large)
    .disabled(speech.loadingText != nil)
    .accessibilityHint("Plays a short companion voice introduction")
  }
