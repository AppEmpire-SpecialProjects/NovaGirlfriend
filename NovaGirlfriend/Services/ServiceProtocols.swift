...
struct AICompletionRequest: Codable, Sendable {
    ...
    /// When false the persona block is omitted. Utility calls such as memory
    /// extraction must behave as neutral assistants, not as the companion.
    var includesPersona = true
    /// Downscaled JPEG attached to the latest user turn (vision requests).
